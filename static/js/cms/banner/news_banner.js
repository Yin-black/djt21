$(function () {
  // 页面一加载 请求api 接口返回数据
  (async () => {
    await $.get('/cms/news/banner/list/', res => {
  ...
  function closeBanner(bannerItem) {
    ...
    $closeBtn.click(function () {
      ...
      ALERT.alertConfirm({
        ...
        "confirmText": "确认",
        "confirmCallback": () => {
          selfAjax("/cms/news/banner/", 'delete', { "banner_id": bannerId}, res=>{
             if (res["code"] === 2) {
  ...
  function uploadBannerImg(bannerItem) {
    ...
    $bannerImageSelect.change(function () {
      ...
      // 发送请求
      $.ajax({
        url: "/cms/news/banner/upload_file/",
        method: "post",
        data: formData,
        // 定义文件的传输
        processData: false,
  ...
  function saveBanner(bannerItem) {
    ...
    $saveBtn.click(function () {
      ...
      let bannerId = bannerItem.data("banner-id");
      let priotiNumber = bannerItem.find('.priority-number');
      ...
      selfAjax("/cms/news/banner/", method, data, res=>{
        if (res["code"] === 2) {
          if (bannerId) {
            ALERT.alertSuccessToast("更新成功");
            //$(".priority-number").text(priority);
             priotiNumber.text(priority);
          } else {
    ...
    if (banner) {
      ...
      bannerStr = `<li class="box banner-item box-primary">
        ...
        <div class="box-body">
          <div class="pull-left banner-img">
            <input type="file" name="banner-image-select" style="display: none;">
            <img src="/static/images/banner/1.png" class="img-thumbnail banner-image">
          </div>
          <div class="pull-left banner-info">
            <div class="form-group">
